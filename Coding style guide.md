# <center>C++ coding style guide </center>
 
 <center>## ⏱ Время на чтение: 20 минут  
...
```cpp
struct apple {
  std::string color; // Не используем нижнее подчеркивание
  int weight;
}
```
...
и содержит в себе атрибуты класса, а так же *объявляет* методы. 
...
Figure(std::string shape, int id) {
...
void paintFigure() {
  // Тут описывается тело функции paintFigure, объявленной в заголовочном файле
  DoPainting()
}
...
``` cpp
class Figure {
    // Пустые коструктор и деструктор
    Figure() {} 
    ~Figure() {}
}
```
